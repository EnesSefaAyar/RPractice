...
# Youtube Workshop (ggplot2)
```{r}
data("faithful")
ggplot(data = faithful, # Global data can be used by other layers
       mapping = aes(x= eruptions, y = waiting)) + # global mapping
  geom_point()

ggplot() +
  geom_point(mapping = aes(x= eruptions, y = waiting), # local mapping
             data = faithful) # data can be used by only geom_point()

ggplot(faithful) + # better to keep data global
  geom_point(mapping = aes(x= eruptions, # asthetics local
                           y = waiting,
                           color = eruptions < 3))

ggplot(faithful) + 
  geom_point(mapping = aes(x= eruptions,
                           y = waiting # if you put color parameter in aes, doesnt work
                           ), color = "steelblue")

ggplot(faithful) + 
  geom_histogram(mapping = aes(x = eruptions, fill = waiting <60), 
                 position = "dodge", 
                 alpha = 0.3) # single position


ggplot(faithful, aes(x = eruptions, y = waiting)) + 
  geom_density_2d() + geom_point(shape = "square", alpha = 0.3)
