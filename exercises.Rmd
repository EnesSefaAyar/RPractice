...
?geom_point
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting), alpha = 0.3, size = 3, shape = "square")
```
...
```{r}
ggplot(faithful) + 
  geom_histogram(mapping = aes(x = eruptions, fill = eruptions < 3.1)) # single position
...
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting)) +
  geom_abline(intercept = 200, slope=-40)
```
...
The stat can be overwritten. If we have pre-computed count we don't want any 
...
ggplot(mpg) + 
  ...
  stat_summary(aes(x = class, y = hwy), fun = mean, geom = "point", 
               color = "red", size = 2)
```
...
p + p + p + plot_layout(widths = c(2,2,3))
```
